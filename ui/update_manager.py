...
class UpdateManager:
    ...
    async def run_startup_maintenance(self) -> None:
        ...
        except Exception:  # noqa: BLE001 - 유지보수 실패는 앱 흐름을 막지 않는다
            pass
    ...
    def _show_available_banner(self, info: Any) -> None:
        ...
        row = ft.Row(
            [
                ...
                ft.TextButton("업데이트", on_click=lambda e: self.app.page.run_task(self._start_download, info)),
                ft.IconButton(
                    icon=ft.Icons.CLOSE,
    ...
    async def _start_download(self, info: Any) -> None:
        ...
        def on_progress(fraction: float) -> None:
            ...
            self.app.page.run_thread(self._apply_download_progress, progress_text, progress_bar, fraction)
    ...
    def _apply_download_progress(self, text_ctl: ft.Text, bar_ctl: ft.ProgressBar, fraction: float) -> None:
    ...
    def _show_ready_banner(self, info: Any) -> None:
        if self.app.is_download_active():
            ...
            row = ft.Row(
                [
                    ft.Icon(ft.Icons.CHECK_CIRCLE, size=18, color=ft.Colors.ON_PRIMARY_CONTAINER),
                    ft.Text("업데이트 준비가 끝났습니다. 지금 재시작할까요?", color=ft.Colors.ON_PRIMARY_CONTAINER, expand=True),
    ...
    def _restart_now(self, info: Any) -> None:
        ...
        if self.app.is_download_active():
            ...
            return
        try:
            self._apply_and_restart(info)  # 정상적으로는 여기서 프로세스가 종료되어 돌아오지 않는다.
        except Exception:  # noqa: BLE001
            pass
        if self.app.demo:
